...
- **콘솔 프레임워크**: core/foundry **1.8.1** — 메타패키지 대신 루트
  `requirements.txt`의 검증된 하위 패키지를 고정한다.
- **Hosted Agent 프레임워크**: core **1.11.0**, foundry/openai **1.10.1**,
  hosting **1.0.0a260709** — 예제별 `requirements.txt`에서 별도 관리한다.
...
- Hosted Agent는 단일 `azure.yaml`, Responses `2.0.0`,
  `azure.ai.agents>=1.0.0-beta.5` 형식을 사용한다. 일반 예제는 `server.run()`,
  비동기 프로바이더가 있는 Foundry IQ는 `await server.run_async()`를 쓴다.
