...
2. **Handoff 워크플로우** (`02_handoff_workflow.py`) — 접수 에이전트가 전문가에게 위임
3. **GroupChat 워크플로우** (`03_group_chat.py`) — 여러 에이전트가 협업 토론
4. **Custom 워크플로우** (`04_custom_workflow.py`) — 조건부 라우팅 순차 파이프라인
...
- **오케스트레이션**: `HandoffBuilder`, `GroupChatBuilder` (`agent_framework.orchestrations`)
- **인증**: `azure-identity` → `AzureCliCredential` (로컬은 `az login` 세션 사용)
- **모델**: Azure AI Foundry 배포 모델 (기본 `gpt-4o`)
...
- Handoff는 `HandoffBuilder(name=..., participants=...).with_start_agent(...).add_handoff(from, [to...]).build()`
