...
- **이 프로젝트는 실습 재현성 및 Beta SDK 안정성을 위해 모든 패키지를 `==`으로 정확히 핀닝한다.**
  (`agent-framework`, `azure-search-documents` 등 프리릴리스 포함)
- 일반 라이브러리를 추가할 때는 `>=`로 최소 버전 지정도 허용하지만, SDK·핵심 패키지는 `==`을 유지한다.
