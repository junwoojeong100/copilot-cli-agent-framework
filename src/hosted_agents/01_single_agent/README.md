...
| `Dockerfile` | 컨테이너 이미지(8088 포트, `linux/amd64`) |
...
# 3) 로컬 호스트 실행 (http://localhost:8088)
azd ai agent run
...
```bash
# (Foundry 프로젝트가 없으면) 리소스 프로비저닝
azd provision

# 컨테이너 빌드 → ACR push → Foundry Agent Service 배포
azd deploy
```
